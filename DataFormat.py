...
class DataFormat():
    ...
    ******'''
    ...
    def __load_gff3__(self,file_name):
        ...
        return df
    '''
    def get_all_fasta_paths(self,file_name):
        fasta_paths = []
        path = '%s/%s.fasta' % (self.fasta_path,file_name)
        for dirpath, dirnames, filenames in os.walk(path):
            for fn in filenames:
                if fn.lower().endswith((".fasta", ".fa", ".fna")):
                    fasta_paths.append(os.path.join(dirpath, fn))
        return fasta_paths
    '''

    def __load_fasta__(self, file_name):
        """Load a single phage genome FASTA into a DataFrame."""
    
        # File like: <path>/<DB>/<Phage_ID>.fasta
        # For you: C:\Users\gabri\OneDrive\Desktop\CompBio Project\TemPhD\TemPhD\<Phage_ID>.fasta
        path = os.path.join(self.fasta_path, f"{file_name}.fasta")
    
        if not os.path.exists(path):
            raise FileNotFoundError(f"FASTA file not found for {file_name}: {path}")
